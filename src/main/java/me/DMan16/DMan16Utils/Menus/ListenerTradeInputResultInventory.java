...
public abstract class ListenerTradeInputResultInventory extends ListenerInventoryPages {
	...
	protected void clearItems() {
		...
		if (Utils.notNull(item)) Utils.givePlayer(player,player.getLocation(),false,item);
		Utils.givePlayer(player,player.getLocation(),false,extraItemsSlots.stream().map(this::getItem).filter(Utils::notNull).toList());
	}
}