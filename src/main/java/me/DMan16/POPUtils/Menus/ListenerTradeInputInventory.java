...
public abstract class ListenerTradeInputInventory extends ListenerInventoryPages {
	protected static final @NotNull NamespacedKey KEY = new NamespacedKey(POPUtilsMain.getInstance(),"trade_input_inventory");
	...
	private static <V extends ListenerTradeInputInventory> boolean first(@NotNull ListenerTradeInputInventory menu,@NotNull Itemable<?> item,@NotNull List<ItemStack> requiredPayment,@Nullable Function<V,@NotNull Boolean> doFirst) {
		menu.item = item;
		menu.slotItem = 4;
		menu.requiredPayment = requiredPayment.stream().filter(i -> !Utils.isNull(i)).toList();
	...
	@NotNull
	protected BasicItemable illegalResultItem() {
		return itemBorder();
	}
	
	@NotNull
	protected BasicItemable legalResultItem() {
		return itemOk();
	}
	
	protected void clearPayments() {
		Utils.givePlayer(player,player.getLocation(),false,paymentSlots.stream().map(this::getItem).filter(item -> !Utils.isNull(item)).toList());
	}
	...
	protected List<ItemStack> checkLegalPayment(@NotNull List<ItemStack> paymentItems) {
		...
		int count = 0;
		for (int i : requiredSlots) if (!Utils.isNull(inv.getItem(i))) count++;
...
//	protected void result(Player player, Inventory inv, List<Integer> resultSlots) {
//		new BukkitRunnable() {
//			public void run() {
//				new ListenerTradePaymentResultInventory(player,inv,itemSlot,null,resultSlots,npc,closeMessage);
//			}
//		}.runTask(POPUtilsMain.getInstance());
//	}
