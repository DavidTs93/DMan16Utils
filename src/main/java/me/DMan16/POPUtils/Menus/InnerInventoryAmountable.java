...
public abstract class InnerInventoryAmountable<V extends Itemable<V> & Amountable<V> > extends InnerInventory<V> {
	...
	protected <T extends InnerInventoryAmountable<V>> InnerInventoryAmountable(@NotNull Player player, @Nullable Component name, @NotNull JavaPlugin plugin, boolean owner,
																			   @NotNull UUID ID, Boolean allowEdit,
	...
	private static <V extends Itemable<V> & Amountable<V>,T extends InnerInventoryAmountable<V>> boolean first(@NotNull InnerInventoryAmountable<V> inner,
																											   @NotNull Function<ItemStack,V> fromItem,
	...
	protected void otherSlot(@NotNull InventoryClickEvent event, int slot, ItemStack slotItem, @NotNull ClickType click) {
		...
		if (slot < size - 9 || slot >= size) {
			if (action == InventoryAction.MOVE_TO_OTHER_INVENTORY) {
				...
				} else if (!Utils.isNull(event.getCursor()) && action.name().startsWith("PLACE_")) {
...
//					new BukkitRunnable() {
//						public void run() {
//							player.setItemOnCursor(Utils.subtract(cursor,finalAmount));
//						}
